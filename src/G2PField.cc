...
int G2PField::Initialize()
{
    ...
    int status = -1;
    if (ReadMap() == 0) status = 0;
    else if (CreateMap() == 0) status = 0;
...
int G2PField::Configure()
{
    ...
    if (!(gConfig->lookupValue("field.origin.x", fOrigin[0])
            && gConfig->lookupValue("field.origin.y", fOrigin[1])
            && gConfig->lookupValue("field.origin.z", fOrigin[2])))
        Warning(here, "Cannot find setting \"field.origin\", using default value ......");

    if (!(gConfig->lookupValue("field.angle.alpha", fEulerAngle[0])
